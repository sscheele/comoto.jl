...
function main(urdf_filepath::String="jaco_urdf.urdf")
    ...
    # end_effector_fn = get_kuka_joint(jaco_tree, "j2s7s300_link_1");
    end_effector_fn = get_jaco_ee_postition_fun(jaco_tree, cache);
    println(end_effector_fn(joint_pos_1));
    println(end_effector_fn(joint_pos_2));
    println(end_effector_fn(joint_pos_3));
end
