function spider_plot(P, E, legend_labels, second_color, varargin)
...
colors = lines(10);  % get a bunch of colors so we can select different ones for tiling
line_style = '-';
line_width = 2;
marker_type = 'o';
marker_size = 8;
...
E_min_scaled = zeros(size(E));
E_max_scaled = zeros(size(E));
...
for ii = 1:num_data_points
    ...
    if isempty(axes_limits)
        ...
        E_min_scaled(:, ii) = (((group_points - group_error_points) - min_value) / range) * (1 - rho_increment) + rho_increment;
        E_max_scaled(:, ii) = (((group_points + group_error_points) - min_value) / range) * (1 - rho_increment) + rho_increment;
    else
...
for ii = 1:theta_end_index
    ...
    for jj = 2:length(rho)
        ...
        % Display axes text
        text_str = sprintf(sprintf('%%.%if', axes_precision), abs(axes_value));
        % offset axis labels, hardcoded for 4 axes
        x_off = 0;
        y_off = 0;
        if mod(ii, 2) == 1
            x_off = -0.12;
        ...
        text(x_axes(jj) + x_off, y_axes(jj) + y_off, text_str,...
            'Units', 'Data',...
            'Color', 'k',...
...
for ii = 1:num_data_groups
    ...
    legend_objs = [legend_objs, p];
    
%     % Error bar test (only works for 4 axes...)
%     err = (E_max_scaled(ii, :) - E_min_scaled(ii, :))/2;
%     for jj = 1:size(err, 2)
%         if mod(jj, 2) == 1
%             errorbar(x_points(jj), y_points(jj), err(jj), 'vertical',...
%                 'LineStyle', line_style,...
%                 'Marker', 'none',...
%                 'Color', colors(ii, :),...
%                 'LineWidth', .667*line_width,...
%                 'CapSize', 14)
%         else
%             errorbar(x_points(jj), y_points(jj), err(jj), 'horizontal',...
%                 'LineStyle', line_style,...
%                 'Marker', 'none',...
%                 'Color', colors(ii, :),...
%                 'LineWidth', .667*line_width,...
...
% plot error bars
% for ii = 1:num_data_groups
%     [xe_min_points, ye_min_points] = pol2cart(theta(1:end-1), E_min_scaled(ii, :));
%     [xe_max_points, ye_max_points] = pol2cart(theta(1:end-1), E_max_scaled(ii, :));
%     for jj = 1:size(xe_min_points, 2)
%         xe = [xe_min_points(jj), xe_max_points(jj)];
%         ye = [ye_min_points(jj), ye_max_points(jj)];
%         plot(xe, ye,...
%         'LineStyle', line_style,...
%         'Marker', 'none',...
%         'Color', colors(ii, :),...
...
plot_handles = findobj(ax.Children, '-not',...
    'Color', grey,...
    '-and', 'Type', 'Line');
err_handles = findobj(ax.Children, '-not',...
    'Color', grey,...
    '-and', 'Type', 'ErrorBar');
...
% Legend (hard-coded)
legend(legend_objs, legend_labels, 'Location', 'southeast');
